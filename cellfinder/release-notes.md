...
* There is a new and improved GUI for manually segmenting linear tracks and volumes in standard space (see [Manually segment in standard space](release-notes.md)).
...
* New all python training data generation interface, `cellfinder_curate`. Details [here](https://sainsburywellcomecentre.github.io/cellfinder/main/user\_guide/training.html).
...
* Cube extraction now supports the output of [ROI sorter](https://github.com/SainsburyWellcomeCentre/Roi\_sorter), rather than just xml.
...
*   The mouse brain atlas can be downloaded to any directory when installing cellfinder using the `--atlas-install-path` flag. This flag can also be used to point to an existing atlas installation (e.g. from [aMAP](https://github.com/SainsburyWellcomeCentre/amap\_python)).
...
* `cellfinder_view_cells2D` function to view cells overlaid on raw data. Uses hardlyany RAM, but I recommend [ROI sorter](https://github.com/SainsburyWellcomeCentre/Roi\_sorter)instead.
...
* Includes (rudimentary) integration of [aMAP](https://github.com/SainsburyWellcomeCentre/amap\_python) for registration to the Allen brain atlas
...
* Output of cell candidate detection step `cells.xml` can now be opened in [Roi Sorter](https://github.com/SainsburyWellcomeCentre/Roi\_sorter)

### Changed

* Docs generated by pydocmd means that installation info is currently [here](https://github.com/adamltyson/cellfinder/tree/master/docs/main/user\_guide)
...
* Cell candidate detection .xml file only provides positive candidate types `<Type>1</Type>` for compatibility with [Roi Sorter](https://github.com/SainsburyWellcomeCentre/Roi\_sorter)
